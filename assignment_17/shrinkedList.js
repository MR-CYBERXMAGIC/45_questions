...
while (invitation.length > 2) {
    var removedGuest = invitation.pop(); // Removes last guest from the list
    console.log("".concat(removedGuest, " sorry for inconvenience. We will plan some thing later"));
}
;
console.log("\nNow I should tell the remaining two that they are still invited\n");
for (var i = invitation.length - 1; i >= 0; i--) {
    console.log("Hello ".concat(invitation[i], ". You are Still invited")); // telling the left 2 that they are still invited.
